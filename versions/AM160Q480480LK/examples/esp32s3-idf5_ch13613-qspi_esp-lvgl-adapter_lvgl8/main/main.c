...
#include "ch13613_init_cmds.h"

static const char *TAG = "LVGL_CH13613_CHSC6417_QSPI";
...
// ==================== CH13613 区域对齐回调 ====================
static void ch13613_area_rounder_cb(lv_area_t *area, void *user_data)
{
    uint16_t x1 = area->x1;
    uint16_t x2 = area->x2;
    uint16_t y1 = area->y1;
...
void app_main(void)
{
    ESP_LOGI(TAG, "ESP32-S3 + CH13613 (QSPI) + CHSC6417 + LVGL8 开始初始化...");

    // 1. QSPI 总线初始化
    spi_bus_config_t bus_config = CH13613_PANEL_BUS_QSPI_CONFIG(
    ...
    // 3. CH13613 Panel 初始化
    ch13613_vendor_config_t vendor_config = {
    ...
    ESP_ERROR_CHECK(esp_lcd_new_panel_ch13613(panel_io, &panel_config, &panel_handle));
    ...
    ESP_ERROR_CHECK(esp_lcd_panel_ch13613_set_brightness(panel_handle, 80));

    ESP_LOGI(TAG, "CH13613 QSPI 显示初始化完成 (%dx%d)", EXAMPLE_LCD_H_RES, EXAMPLE_LCD_V_RES);
    ...
    //注册对齐回调
    ESP_ERROR_CHECK(esp_lv_adapter_set_area_rounder_cb(disp, ch13613_area_rounder_cb, NULL));
